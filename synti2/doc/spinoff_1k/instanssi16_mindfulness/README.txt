...
compo size limit was 4096 bytes, but this packs into only 1024 (using
zopfli with --i3000).
...
-presentation at the party seminar session. So the party version was
something like 1035 or so.
...
- Changed the shader algorithm just a bit. I doubt that anyone will
  notice the slight difference in graphics :). One byte shorter.

- Added some unnecessary code which seems to affect the zopfli packing
  algorithm just enough to squeeze the last byte off the result. Yei.
  1024 Linux 1k intro is finally finished.

Some more bits could maybe have been saved using a combination of the
following tricks, which will definitely never be tried on this one:
