;;; da1k7 - try to improve from previous, precomputed sequence with minimal
;;; footprint but with possibility of some nice instrument sounds.
;;;
;;; Some overall things to try here:
;;; 
;;; 	(1) keep the precalculation of the whole song; audio callback does
;;;         just copying.
;;; 
;;; 	(2) use a MIDI-like song format with time deltas and delta-encoded
;;;         notes, and a special coding for repeats (but see if zopfli even
;;;         needs this kind of "help" .. Answer: compression can do the exact
;;;         thing I was contemplating. If a home-brewed repeat logic
;;;         is to be invented, it has to do something more useful than repetition
;;;         of some preceding events.)
;;;
;;;     (3) tool program to produce sequence data in the own micro format..
;;; 	    at least delta encoding.. manual conversion is getting difficult..
...
;;; Notes:
;;;   - previous synth exe size (zopfli --i250; extraction stub included):
;;;     1084 bytes (8110 bits in deflate stream)
;;;   - first try with delta-encoded notes and repeat logic; same experiment song as previous:
;;;     1110 bytes (8320 bits in deflate stream) The exact same repeat logic appears in deflate!
;;;   - remove the home-made repeat logic that deflate compression can do as well:
;;;     1092 bytes (8175 bits in deflate stream)
;;;   - use a special byte to mark end of layer and song:
;;;     1088 bytes (8144 bits in deflate stream)
;;;   - end marker can be selected arbitrarily.. use a more commonly used byte:
;;;     1087 bytes (8136 bits in deflate stream)
;;;   - no delta encoding but actual notes.. seems simpler for short tunes.
;;;     1083 bytes (8099 bits in deflate stream)
;;;   - more self-similar opcodes (jnz instead of jmp, where applicable):
;;; 	1079 bytes (8072 bits in deflate stream)
;;;   - move note length in register (r14) instead of stack:
;;;     1078 bytes (8058 bits in deflate stream)
;;;   - actually use rbx for both note length and mix data (length inaccurate):
;;;     1077 bytes (8056 bits in deflate stream)
;;;   - revert to actual mul instead of cumulative sum for length computations:
;;;     1073 bytes (8017 bits in deflate stream)
;;;   - observe ZF is naturally set after mix_note; use jz (more common), not jmp:
;;;     1072 bytes (8010 bits in deflate stream)
;;;   - more self-similar opcodes (mov ebx,eax instead of push-pop):
;;;     1071 bytes (8006 bits in deflate stream)
;;;   - lea into 32b register (evade prefixed opcode); revert to relative lea for rbp:
;;;     1069 bytes (7992 bits in deflate stream)
;;;   - there's always some bits "in the rnd seed" after other places have changed:
;;;     1069 bytes (7986 bits in deflate stream)
;;;   - uniform "s"->"u" (0x75) in shader; compress better also on synth side:
;;; 	1068 bytes (7978 bits in deflate stream)
;;;   - more straightforward merging of length and mix bits
;;;     1066 bytes (7962 bits in deflate stream)
;;;   - use only one output channel (leave multiplex to SDL):
;;;     1064 bytes (7945 bits in deflate stream)
;;;   - transpose the bassline by +one semitone; compress better with current code:
;;;     1063 bytes (7938 bits in deflate stream)
;;;   - tweak hash in main headers; it's all in the same deflate soup:
;;;     1062 bytes (7935 bits in deflate stream)
;;;   - don't require nibble duplication in instrument data (values less arbitrary):
;;;     1061 bytes (7925 bits in deflate stream)
;;;   - change program load address to better compressible bytes:
;;;     1059 bytes (7912 bits in deflate stream)
;;;   - change the "makebelieve" size of relocation info in main headers:
;;;     1058 bytes (7904 bits in deflate stream)
;;;   - main changed (remove unused instruction; unrelated to synth; but nice):
;;;     1057 bytes (7894 bits in deflate stream)
;;;   - main changed (revert to calling 'r14++' always; unrelated; but nice):
;;;     1056 bytes (7881 bits in deflate stream)
;;;   - main changed (utilize deflate repeats in ev loop check; unrelated; but nice):
;;;     1055 bytes (7879 bits in deflate stream)
;;;   - main changed (SDL2 specs exploited to the max in ev loop; unrelated):
;;;     1053 bytes (7860 bits in deflate stream)
;;;   - main changed (reordering in renderfunc for deflate repetition; unrelated):
;;;     1053 bytes (7859 bits in deflate stream)
;;;   - switching btw leas, push-pops, movs in synth init (depends on other parts):
;;; 	1051 bytes (7848 bits in deflate stream)
;;;   - some 0-padding to adjust sequence start address (some bytes same in main):
;;; 	1051 bytes (7845 bits in deflate stream)
;;;   - twiddle with 32 and 64 bit instructions.. some longer opcodes compress better:
;;; 	1050 bytes (7834 bits in deflate stream)
;;;   - revert to direct 32b moves instead of push-pop in main (unrelated to synth):
;;;     1047 bytes (7816 bits in deflate stream)
;;;   - use a shorter-opcode lea in audio callback:
;;;     1046 bytes (7801 bits in deflate stream)
;;;   - duplicate xchg of synth callback in main code to streamline register loads:
;;;     1045 bytes (7800 bits in deflate stream)
;;;   - just a little bit out-of-tune to get more compressible reprs of floats.
;;;     1045 bytes (7796 bits in deflate stream)
;;;   - use a more common opcode for dummy popping of stack space.
;;;     1045 bytes (7793 bits in deflate stream)
;;;   - Again! There's always some bits "in the rnd seed" after changes...:
;;;     1044 bytes (7786 bits in deflate stream)
;;;   - And now the 32 bit lea is once again better than 64b. Changes after changes.
;;;     1043 bytes (7784 bits in deflate stream)
;;;   - Micro-organize in main (unrelated). Zopfli wins the lost bit back with --i2000
;;;     1044 bytes (7785 bits in deflate stream)
;;;   - Micro-organize addresses in main (unrelated). Now it's fine with --i250, too.
;;;     1043 bytes (7783 bits in deflate stream)
;;;   - 'rbp-8+byte;add 8' instead of rbp+dw in main (unrelated). More=less with defl.
;;;     1041 bytes (7762 bits in deflate stream)
;;;   - More flexibility via separation of song pattern data. Lose 9 bytes :(
;;;     1050 bytes (7837 bits in deflate stream)
;;;   - Minor correction by tweaking the rng once again.
;;;     1048 bytes (7824 bits in deflate stream)
...
%endif
;;;  	times 0x21	db 0	; Adjust rbp-relative addr to rbp+0x31 (compressible?)
syn_ticklen:
	dd	SYN_TICKLEN
...
;;; 	mov	eax,0x00010003	; 65539 (randu)
   	mov	eax,0x73cbd0c1	; .. or could just repeat suitable bytes..
 	ror	bl,1
