...
vec3(sin(f)*e.x-cos(f)*e.z,e.y,cos(f)*e.x+sin(f)*e.z);}vec3 v(vec3
e,float f){return
vec3(sin(f)*e.x-cos(f)*e.y,cos(f)*e.x+sin(f)*e.y,e.z);}vec3 w(vec3
e,float f){return
vec3(e.x,sin(f)*e.y-cos(f)*e.z,cos(f)*e.y+sin(f)*e.z);}float x(vec3
e,vec2 f){vec2 g=vec2(length(e.xy)-f.x,e.z);return
length(g)-f.y;}float y(vec3 e,vec2 f){e.y-=abs(e.x);return
x(e,vec2(f.x,f.y));}float z(vec3
e){e=v(e,s[0]*.1);e=w(e,s[0]*.2);e=u(e,s[0]);float
f,g;f=y(e+vec3(7,0,0),vec2(5.+s[6],1));e=w(e,3.14+sin(s[0]));g=y(e,vec2(7.+s[6]*2.,1));return
min(f,g);}float A(vec3 e){e=u(e,s[0]);float
f=y(e,vec2(7.+s[6]*2.,1));return f;}float B(vec3
e){if(s[0]<29.6)return A(e);else return z(e);}const float a=.01;const
float b=.1;const int c=180;const float d=80.;vec4 C(vec3 e,vec3
f){float g=0.;int h;vec3 i;for(h=0;h<c;h++){i=e+g*f;float
j=B(i)*.9;g+=j;if(j<a)break;if(g>d)return vec4(i,0);}vec4
j;j.xyz=i;j.w=1.-float(h)/float(c);return j;}vec3 D(vec3 e){return
normalize(vec3(B(e+vec3(b,0,0))-B(e-vec3(b,0,0)),B(e+vec3(0,b,0))-B(e-vec3(0,b,0)),B(e+vec3(0,0,b))-B(e-vec3(0,0,b))));}vec4
E(vec3 e,vec3 f,vec3 g,vec3 h,vec3 i,vec3 j,vec3 k,vec3 l){vec3
m,n,q,r,s,t;m=j;n=normalize(h-f);if(n.z>0.)return vec4(m,1);float
o,p;o=length(h-f);p=1./(1.+.03*o+.003*o*o);q=normalize(f-e);r=k*max(dot(g,n),0.);s=reflect(n,g);t=l*pow(max(dot(s,q),0.),4.);m+=p*i*(r+t);return
vec4(m,1);}void main(){vec3
e,f,h,j,l;e=vec3(0,0,-20.);f=vec3(-10.);vec2
g=gl_FragCoord.xy/vec2(s[1]/2.,s[2]/2.)-vec2(1);g.x*=s[1]/s[2];h=vec3(g.x,g.y,1);h=normalize(h)*.5;vec4
i=C(e,h);j=i.xyz;float k=i.w;l=D(j);if(k>0.){vec3
m,n,o,p;m=vec3(3);n=vec3(.3,0,0);o=vec3(1,0,0);p=vec3(1,1,0);vec4
q=E(e,j,l,f,m,n,o,p);gl_FragColor=q*k;}else if(s[0]>15.){vec3
m=v(vec3(g,0),.2*s[0]);gl_FragColor=vec4(sin(s[0])*cos(4.*m.x*m.y)+s[3]);}else
