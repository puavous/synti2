...
static void render_scene(const synti2_synth *s){
  ...
  */
  ...
  //   glClearColor (cf, cf, cf, 0.0);
   ...
   glTranslatef(0.f, 0.f , -2.f); // +100. * sin(time) - .1*s->note[0]);
   //  glRotatef(90.f, 0.f, 0.f , 1.f); // +100. * sin(time) - .1*s->note[0]);

   //glRotatef(time*4.f + time*(1.f+s->note[4]), 1.f, s->note[4] % 3, 0.f);
   glRotatef(time*16.f, 0.f, 1.f, 0.f);
