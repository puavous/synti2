...
typedef struct {
  ...
  int voices[NUM_CHANNELS+1];       /* unison duplicates / polyphony (1..#channels)*/
  ...
  int mod_src[NUM_MODULATORS];  /* 128 controllers (MIDI) to internal controls. */
  float mod_min[NUM_MODULATORS]; /* destination-specific min value. */
...
typedef struct {
  int inxt;
  int rot_on[NUM_CHANNELS];
} poly_rotation_state;

typedef struct {
  int prev_note;  /* Previous note on. */
  int ons[128]; /* Voices of note-ons (for poly)*/
  int notes[NUM_CHANNELS]; /* Notes of voices (for poly)*/
